...
figure('color','w');
surf(X,Y,exp(Y),'facecolor',[50 50 50]/255,'facealpha',0.3)
zlim([-10 10])
title('f(x,y)=e^x')
hold on;
...
for i_order = 1:n_order
    syms X Y;
    my_f = feval(symengine,'mtaylor',exp(Y), ['[X=',num2str(app_point(1)),', Y=',num2str(app_point(2)),']'] ,i_order); 
    ...
    plot3(app_point(1), app_point(2), exp(app_point(2)),'r.','markersize',10);
    line([app_point(1) app_point(1)],[-5 5],[-10 -10],'color','r','linestyle','--')
    line([-5 5],[app_point(2) app_point(2)],[-10 -10],'color','r','linestyle','--')
    line([app_point(1) app_point(1)],[app_point(2) app_point(2)],[-10, exp(app_point(2))],'color','r','linestyle','--')
    zlim([-10 10])
    xlabel('x');
    ylabel('y');
    [caz, cel] = view;
